...
-- Function to check if a user is an accepted organizer of a personal event with edit permission
...
DROP POLICY IF EXISTS "Organizers can view food items" ON personal_event_food_items;
...
CREATE POLICY "Organizers can view food items"
ON personal_event_food_items
FOR SELECT
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM personal_event_food_sessions fs
    WHERE fs.id = personal_event_food_items.food_session_id
    AND is_personal_event_organizer(auth.uid(), fs.event_id)
  )
);
...
USING (
  EXISTS (
    SELECT 1 FROM personal_event_food_sessions fs
    WHERE fs.id = personal_event_food_items.food_session_id
    AND has_guest_view_access(fs.event_id, auth.uid(), 'food')
  )
);